...
@section('title', 'Edit Trigger Email')
@section('page-title', 'Edit Trigger Email')

@section('content')
<div class="max-w-2xl">
    <a href="{{ route('admin.crm.triggers.index') }}"
    ...
    <div class="flat-card p-6">
        <h2 class="font-mono text-sm font-semibold text-slate-800 dark:text-white mb-5">Edit Trigger: {{ $trigger->name }}</h2>

        <form action="{{ route('admin.crm.triggers.update', $trigger) }}" method="POST">
            @csrf @method('PATCH')

            @include('admin.crm.triggers._form')

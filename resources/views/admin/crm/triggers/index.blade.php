...
@section('title', 'Trigger Otomatis')
@section('page-title', 'Trigger Otomatis')

@section('content')
<div class="mb-4 flex items-center justify-between">
    <p class="text-sm text-slate-500 dark:text-slate-400">Email dikirim otomatis berdasarkan event user</p>
    <a href="{{ route('admin.crm.triggers.create') }}"
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="border-b border-slate-100 dark:border-white/[0.05]">
                ...
                <th class="px-5 py-3 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Template</th>
                <th class="px-5 py-3 text-center text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Kondisi</th>
        ...
        <tbody>
            ...
            <tr class="group border-b border-slate-50 dark:border-white/[0.03] hover:bg-slate-50/70 dark:hover:bg-white/[0.02] transition-colors">
                <td class="px-5 py-3">
                    ...
                    @if($trigger->description)
                        <div class="text-slate-400 text-[11px] mt-0.5">{{ $trigger->description }}</div>
                    @endif
                </td>
                <td class="px-5 py-3">
                    @php
                ...
                <td class="px-5 py-3 text-center">
                    <form action="{{ route('admin.crm.triggers.toggle', $trigger) }}" method="POST">
                        @csrf @method('PATCH')
                        <button type="submit" class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none cursor-pointer
                            {{ $trigger->is_active ? 'bg-blue-600' : 'bg-slate-200 dark:bg-slate-700' }}">
                ...
                <td class="px-5 py-3 text-right">
                    <div class="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <a href="{{ route('admin.crm.triggers.edit', $trigger) }}"
                        ...
                        <form action="{{ route('admin.crm.triggers.destroy', $trigger) }}" method="POST"
            ...
            <tr>
                <td colspan="8" class="px-5 py-8 text-center text-sm text-slate-400">
                    Belum ada trigger. <a href="{{ route('admin.crm.triggers.create') }}" class="text-blue-500 hover:underline">Buat trigger pertama</a>.
                </td>
            </tr>
            @endforelse
        </tbody>
...
<div class="mt-4 flat-card p-4">
    <p class="text-xs text-slate-500 dark:text-slate-400">
        ...
        Trigger dijalankan otomatis setiap hari pukul 07:00 WIB via scheduler (<code class="font-mono bg-slate-100 dark:bg-white/5 px-1 rounded">php artisan triggers:run</code>).

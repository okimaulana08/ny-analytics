...
@section('title', 'Edit Grup Email')
@section('page-title', 'Edit Grup Email')

@section('content')
<div class="max-w-2xl">
    <a href="{{ route('admin.crm.groups.index') }}"
    ...
    <div class="flat-card p-6">
        <h2 class="font-mono text-sm font-semibold text-slate-800 dark:text-white mb-5">Edit Grup: {{ $group->name }}</h2>

        <form action="{{ route('admin.crm.groups.update', $group) }}" method="POST" class="space-y-4" id="group-form">
            @csrf @method('PUT')

            @include('admin.crm.groups._form', ['group' => $group])
...
@push('scripts')
@include('admin.crm.groups._form_script')
@endpush
